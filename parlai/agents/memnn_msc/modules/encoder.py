...
class TransformerEncoder(nn.Module):
    ...
    def forward(  # type: ignore
        ...
        positions: Optional[torch.LongTensor] = None,
        segments: Optional[torch.LongTensor] = None,
        hist_aware_cxt_vec: Optional[Tuple[torch.FloatTensor, Optional[torch.BoolTensor, torch.FloatTensor]]] = None,
        **kwargs,
        ...
        :param LongTensor[batch,seqlen] segments:
            If provided, additionally adds ``segments`` as extra embedding features.
        :param FloatTensor[batch, ] hist_aware_cxt_vec:
            If provided, hist_aware_cxt_vec
        """
        # embed input
        tensor, mask = self.forward_embedding(input, positions, segments)
        if hist_aware_cxt_vec is not None:
            tensor = torch.cat([hist_aware_cxt_vec[0], tensor], dim=1)
            mask = torch.cat([hist_aware_cxt_vec[1], mask], dim=1)
